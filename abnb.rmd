...
```{r, load dependencies, warning=FALSE, comment=FALSE, message=FALSE, cache=FALSE}
library(RSocrata)
library(knitr)
library(magrittr)
library(dplyr)
...
```{r, load data, warning=FALSE, comment=FALSE, message=FALSE, cache=FALSE}
...
```{r, warning=FALSE, comment=FALSE, message=FALSE, cache=FALSE}
...
#2. Find only the actual license numbers

pattern <- "^\\d{2}STR-\\d{5}"
library(stringr)
inside.airbnb <- inside.airbnb %>% 
  mutate(license.match = str_match(inside.airbnb$license, pattern))
